...
gs_plugin_setup (GsPlugin *plugin, GCancellable *cancellable, GError **error)
{
        ...
        if (uname (&priv_uname) != 0)
                return FALSE;
        g_stpcpy (priv->os_arch, priv_uname.machine);
        ...
        g_debug ("SL major version detected: %lu", priv->major_v);
        g_debug ("SL minor version detected: %lu", priv->current_minor_v);

        priv->url = g_strjoin ("/", SCIENTIFIC_LINUX_BASEURL,
                               tokens[0], priv->os_arch,
                               "os/repodata/repomd.xml", NULL);
...
_ensure_cache (GsPlugin *plugin, GCancellable *cancellable, GError **error)
{
        ...
        if (!xb_builder_source_load_file (source,
                                         g_file_parse_name (priv->cachefn),
        ...
        silo = xb_builder_ensure (builder,
                                  g_file_parse_name (priv->cachefn_b),
        ...
        if (tmp == NULL) {
            g_debug ("SL cache file: no provided revision/version");
            return FALSE;
        }
        ...
        if (priv->major_v != tmp_int) {
            g_debug ("SL cache file: wrong major version: %lu != %lu",
...
_create_upgrade_from_info (GsPlugin *plugin, GsPluginData *priv)
{
        ...
        /* search in the cache */
        cache_key = g_strdup_printf ("release-%lu.%lu",
                                     priv->major_v,
        ...
        app_id = g_strdup_printf ("org.scientificlinux.SL%lu.%lu-update",
                                  priv->major_v,
                                  priv->published_minor_v);
        app_version = g_strdup_printf ("%lu.%lu", 
                                  priv->major_v,
                                  priv->published_minor_v);
        ...
        gs_app_set_name (app, GS_APP_QUALITY_LOWEST, priv->os_name);
        ...
        url = g_strdup_printf ("http://ftp.scientificlinux.org/linux/scientific/%lu/%s/release-notes/",
