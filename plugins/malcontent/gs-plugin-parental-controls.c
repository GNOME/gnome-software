...
query_app_filter (GCancellable  *cancellable,
                  GError       **error)
{
	/* TODO add dependency on 0.3.0 */
	return mct_get_app_filter (NULL, getuid (),
				   MCT_GET_APP_FILTER_INTERACTIVE, cancellable,
				   error);
}
...
gs_plugin_setup (GsPlugin *plugin, GCancellable *cancellable, GError **error)
{
	...
	priv->app_filter = query_app_filter (cancellable, error);
