 ...
 */
static GHashTable *
gs_plugin_flatpak_group_apps_by_installation (GsPlugin *plugin,
                                              GsAppList *list)
{
                  ...
                  GError **error)
{
	...
	applist_by_flatpaks = gs_plugin_flatpak_group_apps_by_installation (plugin, list);
	...
	while (g_hash_table_iter_next (&iter, &key, &value)) {
		...
		g_return_val_if_fail (flatpak != NULL, FALSE);
		g_return_val_if_fail (list_tmp != NULL, FALSE);
		g_return_val_if_fail (gs_app_list_length (list_tmp) > 0, FALSE);
