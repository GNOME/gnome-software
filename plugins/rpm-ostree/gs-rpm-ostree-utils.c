...
/**
 * gs_rpm_ostree_utils_split_changelogs:
 * @owner_app: a #GsApp
 * @changelogs: (in) (nullable): raw changelogs returned by rpm-ostree
 *
 * Splits @changelogs text into respective apps and adds them
 * into the @owner_app as related apps.
 *
 * The @changelogs is an output of `rpm-ostree db diff --changelogs --format=block` command.
 *
 * Note: This also modifies the @changelogs in place.
 *
 * Since: 50
 **/
void
gs_rpm_ostree_utils_split_changelogs (GsApp *owner_app,
				      gchar *changelogs)
{
