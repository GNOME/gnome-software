			 ...
			 GError **error)
{
	...
	/* set content rating */
	if (flags & GS_PLUGIN_REFINE_FLAGS_REQUIRE_SCREENSHOTS) { //FIXME: need REQUIRE_CONTENT_RATING
	...
	/* set categories */
	if (flags & GS_PLUGIN_REFINE_FLAGS_REQUIRE_MENU_PATH) {
	...
	/* set project group */
	if ((flags & GS_PLUGIN_REFINE_FLAGS_REQUIRE_SCREENSHOTS) > 0 &&
	    gs_app_get_project_group (app) == NULL) { //FIXME: need REQUIRE_PROJECT_GROUP
	...
	/* set developer name */
	if ((flags & GS_PLUGIN_REFINE_FLAGS_REQUIRE_SCREENSHOTS) > 0 &&
	    gs_app_get_developer_name (app) == NULL) { //FIXME: need REQUIRE_DEVELOPER_NAME
	...
	/* add kudos */
	if (flags & GS_PLUGIN_REFINE_FLAGS_REQUIRE_SCREENSHOTS) {//FIXME: should be REQUIRE_KUDOS
	...
	if (flags & GS_PLUGIN_REFINE_FLAGS_REQUIRE_UPDATE_DETAILS) {
			  ...
			  GError **error)
{
	...
	g_warning ("search took %fms", g_timer_elapsed (timer, NULL) * 1000);
	return TRUE;
}
...
void
gs_appstream_add_extra_info (GsPlugin *plugin, XbNode *component)
{
#if 0
// FIXME: this needs to be moved to the XbBuilderSourceFunc fixups to be done
// BEFORE the blob is compiled */

	const gchar *tmp;
	g_autoptr(AsIcon) icon = NULL;

	/* add more search terms */
	switch (xb_node_query_text (component, "kind", NULL)) {
	case AS_APP_KIND_WEB_APP:
	case AS_APP_KIND_INPUT_METHOD:
		tmp = as_app_kind_to_string (xb_node_query_text (component, "kind", NULL));
		g_debug ("adding keyword '%s' to %s",
			 tmp, xb_node_query_text (component, "unique_id", NULL));
