...
gs_plugin_add_language_packs (GsPlugin *plugin,
                       ...
                       GError **error)
{
  const gchar *identified_lang_id = NULL;
  const gchar *identified_langpack_source = NULL;
  const gchar *lang_code_from_locale = NULL;
  ...
  if (g_strrstr(language_code, "_") &&
      !g_hash_table_lookup(locale_langpack_map, language_code)) {

    /* language_code is locale and not found in locale_langpack_map */
    lang_code_from_locale = g_strsplit(language_code, "_", 2)[0];
    g_debug ("lang_code_from_locale %s ", lang_code_from_locale);
