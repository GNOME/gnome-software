...
struct GsPluginData {
	MctAppFilter	*app_filter;  /* (atomic) (owned) (nullable) */
};
...
static gboolean
parental_filter_if_needed (GsPlugin *plugin, GsApp *app, MctAppFilter *app_filter)
{
...
static MctAppFilter *
query_app_filter (GCancellable  *cancellable,
                  GError       **error)
{
	return mct_get_app_filter (NULL, getuid (), TRUE, cancellable, error);
}
...
gs_plugin_initialize (GsPlugin *plugin)
{
	...
	priv->app_filter = query_app_filter (NULL, &local_error);

	if (local_error != NULL) {
		g_warning ("Failed to load parental controls filter: %s",
			   local_error->message);
	}
	...
	gs_plugin_set_appstream_id (plugin, "org.gnome.Software.Plugin.ParentalControls");
}
		      ...
		      GError **error)
{
	...
	/* Filter by various parental filters. If the filter is %NULL it’s
	 * because the plugin failed to start up (and will have emitted a
	 * warning at the time).*/
	app_filter = g_atomic_pointer_get (&priv->app_filter);
	if (app_filter == NULL)
		return TRUE;

	if (parental_filter_if_needed (plugin, app, app_filter))
		return TRUE;
		   ...
		   GError **error)
{
	...
	new_app_filter = query_app_filter (cancellable, error);
	...
	do {
		old_app_filter = g_atomic_pointer_get (&priv->app_filter);
	} while (!g_atomic_pointer_compare_and_exchange (&priv->app_filter,
							 old_app_filter,
							 new_app_filter));
	new_app_filter = NULL;  /* ownership transferred above */
