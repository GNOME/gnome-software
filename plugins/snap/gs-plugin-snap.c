...
static GsApp *
snap_to_app (GsPlugin *plugin, SnapdSnap *snap, const gchar *channel)
{
	GsPluginData *priv = gs_plugin_get_data (plugin);
	GStrv common_ids;
	...
	case SNAPD_SNAP_TYPE_APP:
		unique_id = g_strdup_printf ("system/snap/*/desktop/%s/%s", appstream_id, channel);
		break;
	case SNAPD_SNAP_TYPE_KERNEL:
	case SNAPD_SNAP_TYPE_GADGET:
	case SNAPD_SNAP_TYPE_OS:
		unique_id = g_strdup_printf ("system/snap/*/runtime/%s/%s", appstream_id, channel);
		break;
        default:
	case SNAPD_SNAP_TYPE_UNKNOWN:
		unique_id = g_strdup_printf ("system/snap/*/*/%s/%s", appstream_id, channel);
		break;
	}
	...
	if (channel != NULL)
		gs_app_set_metadata (app, "snap::channel", channel);
			  ...
			  GError **error)
{
	...
	for (guint i = 0; i < channels->len; i++) {
		...
		g_autoptr(GsApp) a = snap_to_app (plugin, snap, snapd_channel_get_name (channel));
		gs_app_list_add (list, a);
	}
	// FIXME: Sort channel list
		      ...
		      GError **error)
{
	...
	channel = gs_app_get_metadata_item (app, "snap::channel");
