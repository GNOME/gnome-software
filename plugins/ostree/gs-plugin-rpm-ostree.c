...
gs_plugin_setup (GsPlugin *plugin, GCancellable *cancellable, GError **error)
{
	...
	if (priv->os_proxy == NULL) {
		...
		g_print ("RPM-OSTREE PLUGIN: set %s as Booted OS\n", os_object_path);
	}
                   ...
                   GError **error)
{
	...
	if (flags & GS_PLUGIN_REFRESH_FLAGS_METADATA) {
		...
		if (!gs_rpmostree_os_call_download_update_rpm_diff_sync (priv->os_proxy,
		                                                         ...
		                                                         error)) {
			gs_utils_error_convert_gio (error);
			g_print ("RPM-OSTREE PLUGIN: error in download_update_rpm_diff: %s\n", (*error)->message);
			return FALSE;
		}
		g_print ("RPM-OSTREE PLUGIN: finished download_update_rpm_diff; transaction_address: %s\n", transaction_address);
		...
		if (!gs_rpmostree_transaction_get_response_sync (priv->sysroot_proxy,
		                                                 ...
		                                                 error)) {
			gs_utils_error_convert_gio (error);
			g_print ("RPM-OSTREE PLUGIN: error in transaction_get_response_sync: %s\n", (*error)->message);
			return FALSE;
		}
		g_print ("RPM-OSTREE PLUGIN: finished get_response after download_update_rpm_diff; transaction_address: %s\n", transaction_address);
	}
