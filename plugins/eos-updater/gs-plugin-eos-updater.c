 ...
 * new update.
 *
 * Calling GsPluginClass::upgrade_download_async() will result in this plugin calling
 * a sequence of methods on the `eos-updater` daemon to check for, download and
 * apply an update. Typically, GsPluginClass::upgrade_download_async() should be called
...
static void
gs_plugin_eos_updater_upgrade_download_async (GsPlugin                    *plugin,
					      GsApp                       *app,
					      ...
					      gpointer                     user_data)
{
	...
	task = g_task_new (plugin, cancellable, callback, user_data);
	g_task_set_source_tag (task, gs_plugin_eos_updater_upgrade_download_async);
...
static gboolean
gs_plugin_eos_updater_upgrade_download_finish (GsPlugin      *plugin,
                                               GAsyncResult  *result,
                                               GError       **error)
{
	return g_task_propagate_boolean (G_TASK (result), error);
...
gs_plugin_eos_updater_class_init (GsPluginEosUpdaterClass *klass)
{
	...
	plugin_class->upgrade_download_async = gs_plugin_eos_updater_upgrade_download_async;
	plugin_class->upgrade_download_finish = gs_plugin_eos_updater_upgrade_download_finish;
}
