...
gs_dkms_complete_refine_task (GTask *in_task)
{
	...
	for (guint i = 0; i < gs_app_list_length (list); i++) {
		...
		if ((check_akmods_key && data->akmods_key_state == GS_DKMS_STATE_ENROLLED) ||
			...
			if ((check_akmods_key && data->akmods_key_state == GS_DKMS_STATE_PENDING) ||
			    (check_dkms_key && data->dkms_key_state == GS_DKMS_STATE_PENDING))
				gs_app_set_metadata (app, "GnomeSoftware::dkms-pending", "True");
			else
				gs_app_set_metadata (app, "GnomeSoftware::dkms-pending", NULL);
