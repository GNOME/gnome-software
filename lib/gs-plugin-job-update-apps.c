...
static gboolean
gs_plugin_job_update_apps_get_interactive (GsPluginJob *job)
{
	GsPluginJobUpdateApps *self = GS_PLUGIN_JOB_UPDATE_APPS (job);
	return (self->flags & GS_PLUGIN_UPDATE_APPS_FLAGS_INTERACTIVE) != 0;
}
...
gs_plugin_job_update_apps_class_init (GsPluginJobUpdateAppsClass *klass)
{
	...
	job_class->get_interactive = gs_plugin_job_update_apps_get_interactive;
