           ...
           GError *error)
{
	...
	/* Icon content implies metadata. */
	if (require_flags & GS_PLUGIN_REFINE_REQUIRE_FLAGS_ICON_CONTENT)
		require_flags |= GS_PLUGIN_REFINE_REQUIRE_FLAGS_ICON_METADATA;

	/* …but we delay fetching the icon content until after filtering, which
	 * we can do because it’s done via the GsIconDownloader rather than
	 * actually part of the refine job. */
	require_flags &= ~GS_PLUGIN_REFINE_REQUIRE_FLAGS_ICON_CONTENT;
             ...
             GsAppList *merged_list)
{
	...
	if (refine_require_flags & GS_PLUGIN_REFINE_REQUIRE_FLAGS_ICON_CONTENT) {
