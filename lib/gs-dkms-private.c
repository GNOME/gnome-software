 ...
 *
 * Returns: (transfer full): a path where DKMS key is stored
 *
 * Since: 47
 **/
gchar *
...
{
	gchar *filename = gs_dkms_get_dkms_key_filename ();
	gchar *ptr = strrchr (filename, G_DIR_SEPARATOR);
	if (ptr != NULL)
		*ptr = '\0';
	return filename;
}
 ...
 * when no longer needed.
 *
 * Returns: (transfer full): a file name of the DKMS key
 *
 * Since: 47
 **/
gchar *
