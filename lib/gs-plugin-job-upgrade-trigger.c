...
/**
 * SECTION:gs-plugin-job-upgrade-trigger
 * @short_description: A plugin job on an app
 *
 * #GsPluginJobUpgradeTrigger is a #GsPluginJob to trigger the distribution
 * upgrade to be installed on next boot.
 *
 * This class is a wrapper around #GsPluginClass.upgrade_trigger_async
...
struct _GsPluginJobUpgradeTrigger
{
	GsPluginJob parent;
...
G_DEFINE_TYPE (GsPluginJobUpgradeTrigger, gs_plugin_job_upgrade_trigger, GS_TYPE_PLUGIN_JOB)

typedef enum {
	PROP_FLAGS = 1,
	PROP_APP,
} GsPluginJobUpgradeTriggerProperty;

static GParamSpec *props[PROP_APP + 1] = { NULL, };

static void
gs_plugin_job_upgrade_trigger_dispose (GObject *object)
{
	GsPluginJobUpgradeTrigger *self = GS_PLUGIN_JOB_UPGRADE_TRIGGER (object);
	...
	G_OBJECT_CLASS (gs_plugin_job_upgrade_trigger_parent_class)->dispose (object);
}

static void
gs_plugin_job_upgrade_trigger_get_property (GObject *object,
					    guint prop_id,
					    GValue *value,
					    GParamSpec *pspec)
{
	...
	switch ((GsPluginJobUpgradeTriggerProperty) prop_id) {
	case PROP_FLAGS:
...
static void
gs_plugin_job_upgrade_trigger_set_property (GObject *object,
					    guint prop_id,
					    const GValue *value,
					    GParamSpec *pspec)
{
...
static void
gs_plugin_job_upgrade_trigger_run_async (GsPluginJob         *job,
					 ...
					 gpointer             user_data)
{
	...
	g_task_set_source_tag (task, gs_plugin_job_upgrade_trigger_run_async);
	...
	for (guint i = 0; i < plugins->len; i++) {
		...
		if (!gs_plugin_get_enabled (plugin))
			continue;
		if (plugin_class->upgrade_trigger_async == NULL)
			continue;
		...
		/* run the plugin */
		self->n_pending_ops++;
		plugin_class->upgrade_trigger_async (plugin, self->app, self->flags, cancellable, plugin_app_func_cb, g_object_ref (task));
	}
		    ...
		    gpointer      user_data)
{
	...
	success = plugin_class->upgrade_trigger_finish (plugin, result, &local_error);
...
finish_op (GTask  *task,
           GError *error)
{
	GsPluginJobUpgradeTrigger *self = g_task_get_source_object (task);
...
static gboolean
gs_plugin_job_upgrade_trigger_run_finish (GsPluginJob   *self,
					  GAsyncResult  *result,
					  GError       **error)
{
...
static void
gs_plugin_job_upgrade_trigger_class_init (GsPluginJobUpgradeTriggerClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GsPluginJobClass *job_class = GS_PLUGIN_JOB_CLASS (klass);

	object_class->dispose = gs_plugin_job_upgrade_trigger_dispose;
	...
	/**
	 * GsPluginJobUpgradeTrigger:flags:
	 *
	...
	props[PROP_FLAGS] =
		g_param_spec_flags ("flags", "Flags",
				    "Flags affecting how the operation runs.",
				    GS_TYPE_PLUGIN_UPGRADE_TRIGGER_FLAGS,
				    GS_PLUGIN_UPGRADE_TRIGGER_FLAGS_NONE,
				    G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
				    G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);
...
static void
gs_plugin_job_upgrade_trigger_init (GsPluginJobUpgradeTrigger *self)
{
}

/**
 * gs_plugin_job_upgrade_trigger_new:
 * @app: (transfer none): an app to run the operation on
 * @flags: flags affecting how the operation runs
 *
 * Create a new #GsPluginJobUpgradeTrigger to cancel offline update of the given @app.
 *
 * Returns: (transfer full): a new #GsPluginJobUpgradeTrigger
 * Since: 47
 */
GsPluginJob *
gs_plugin_job_upgrade_trigger_new (GsApp *app,
				   GsPluginUpgradeTriggerFlags flags)
{
