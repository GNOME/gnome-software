                                ...
                                gpointer             user_data)
{
	...
	g_object_set_data_full (G_OBJECT (task), "gs-plugin-loader", g_object_ref (plugin_loader), g_object_unref);
            ...
            GsAppList *result_list)
{
	...
	if ((self->job_flags & GS_PLUGIN_REFINE_FLAGS_DISABLE_ICONS) == 0 &&
	    (self->require_flags & GS_PLUGIN_REFINE_REQUIRE_FLAGS_ICON) != 0) {
