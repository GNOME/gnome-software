...
{
	const gchar *management_plugin = gs_app_get_management_plugin (self->app);
	gboolean show_button_menu = FALSE;

	if (management_plugin != NULL) {
		GsPlugin *plugin = gs_plugin_loader_find_plugin (self->plugin_loader, management_plugin);
		if (plugin != NULL) {
                                ...
                                GsDetailsPage *self)
{
	...
	if (logged_in) {
		g_autofree gchar *signin_label = g_strdup_printf (_("Signed in as %s"), gs_auth_get_username (auth));
	...
	gtk_popover_set_relative_to (GTK_POPOVER (self->popover_account), GTK_WIDGET (self->button_menu));
	gtk_popover_popup (GTK_POPOVER (self->popover_account));
}
...
gs_details_page_class_init (GsDetailsPageClass *klass)
{
	...
	gtk_widget_class_bind_template_child (widget_class, GsDetailsPage, popover_account);
