...
gs_dkms_dialog_init (GsDkmsDialog *self)
{
	...
	/* generate the password, ideally with each number being used only once */
	rand = g_rand_new_with_seed ((guint32) g_get_real_time ());
	for (guint i = 0; i < PASSWORD_LEN; i++) {
		gchar candidate = '0' + g_rand_int_range (rand, 1, 10);

		/* ensure uniqueness, when the numbers can be unique */
		if (i < 9) {
			gboolean used;
			...
			do {
				...
				for (guint j = 0; j < i && !used; j++) {
					used = self->password[j] == candidate;
					if (used) {
						candidate++;
						/* intentionally skip '0', to not be mixed with capital "o" */
						if (candidate > '9')
							candidate = '1';
					}
				}
