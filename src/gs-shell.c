...
gs_shell_setup (GsShell *shell, GsPluginLoader *plugin_loader, GCancellable *cancellable)
{
	...
	g_autoptr(GMenu) menu_section = NULL;
	g_autoptr(GMenuItem) sources_item = NULL;
	GPtrArray *auth_array;
	...
	g_signal_connect (widget, "clicked",
	                  ...
	                  shell);
	menu = g_menu_new ();
	priv->accounts_menu = g_menu_new ();
	g_menu_append_section (menu, NULL, G_MENU_MODEL (priv->accounts_menu));
	menu_section = g_menu_new ();
	sources_item = g_menu_item_new (_("_Software Repositories"), "app.sources");
	g_menu_item_set_attribute (sources_item, "hidden-when", "s", "action-disabled");
	g_menu_append_item (menu_section, sources_item);
	g_menu_append (menu_section, _("_About"), "app.about");
	g_menu_append_section (menu, NULL, G_MENU_MODEL (menu_section));
	...
	for (guint i = 0; i < auth_array->len; i++) {
		...
		g_menu_append_section (priv->accounts_menu, NULL, G_MENU_MODEL (auth_menu));
...
gs_shell_dispose (GObject *object)
{
	...
	g_clear_object (&priv->accounts_menu);
