...
{
	const gchar *name;
	AdwNavigationPage *visible_page = adw_navigation_view_get_visible_page (shell->details_navigation_view);

	if (g_strcmp0 (adw_view_stack_get_visible_child_name (shell->stack_loading), "loading") == 0)
		return GS_SHELL_MODE_LOADING;

	if (visible_page == adw_navigation_view_find_page (shell->details_navigation_view, "details"))
		return GS_SHELL_MODE_DETAILS;

	if (visible_page == adw_navigation_view_find_page (shell->details_navigation_view, "main"))
