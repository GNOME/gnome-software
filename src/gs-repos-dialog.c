...
enable_repo (GsReposDialog *dialog, GsApp *repo)
{
	...
	if (gs_app_get_description (repo) != NULL) {
		GtkWidget *confirm_dialog;
		g_autofree gchar *message = NULL;
		g_autoptr(GError) error = NULL;

		/* convert from AppStream markup */
		message = as_markup_convert_simple (gs_app_get_description (repo), &error);
		if (message == NULL) {
			message = g_strdup_printf ("Failed to format agreement text: %s",
						   error->message);
		}
		...
		gtk_dialog_add_button (GTK_DIALOG (confirm_dialog), _("Accept"),
				       GTK_RESPONSE_OK);
