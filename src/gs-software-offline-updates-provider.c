		   ...
		   gpointer user_data)
{
	...
	if (gs_plugin_loader_job_process_finish (self->plugin_loader, result, (GsPluginJob **) &plugin_job, &local_error))
		...
		g_debug ("Failed to get offline update state: %s", local_error->message);
		...
		gpointer user_data)
{
	...
	if (!gs_plugin_loader_job_process_finish (self->plugin_loader, result, NULL, &local_error))
		g_debug ("Failed to cancel offline update: %s", local_error->message);
		    ...
		    gpointer user_data)
{
	...
	if (!gs_plugin_loader_job_process_finish (self->plugin_loader, result, NULL, &local_error))
		g_debug ("Failed to set offline update action: %s", local_error->message);
		   ...
		   gpointer user_data)
{
	...
	if (g_strcmp0 (action, "reboot") == 0)
		...
		g_warning ("Unknown offline update action '%s', expects 'reboot' or 'shutdown'", action);
